      ...
      <div className="training-workspace">
        <main className="training-builder">
          ...
          {selectedDay && <section className="training-day-editor">
            ...
            {selectedDay.exercises.length === 0 ? <div className="training-day-empty"><Dumbbell size={25}/><p>Agrega ejercicios desde la biblioteca basada en tu Excel.</p><button onClick={() => { setReplacing(null); setAddedExerciseCount(0); setLibraryOpen(true); }}><Plus size={15}/>Elegir ejercicio</button></div> : <div className="training-exercises">{selectedDay.exercises.map((exercise, index) => {
        ...
        <aside className="training-analysis"><div className="training-analysis-head"><span>SEMANA {week}</span><h2>Revisión del bloque</h2><p>Control rápido antes de entregar la rutina.</p></div><div className={`training-quality ${issues.length ? "warning" : "ok"}`}>{issues.length ? <AlertTriangle size={17}/> : <CheckCircle2 size={17}/>}<div><strong>{issues.length ? `${issues.length} puntos por revisar` : "Semana consistente"}</strong>{issues.map(issue => <p key={issue}>{issue}</p>)}{!issues.length && <p>Rangos y estructura listos para revisión del coach.</p>}</div></div><div className="training-rir-note"><strong>CRITERIO RIR</strong><p>RIR indica cuántas repeticiones adicionales estimas poder realizar. Inicio y final describen la primera y la última serie de trabajo. Acercarse al fallo no obliga a llegar al fallo en todas las series.</p></div><p className="training-evidence">Referencia general: para hipertrofia, la evidencia actual favorece volumen semanal suficiente y progresión gradual. DietForge muestra datos; la decisión final sigue siendo del coach.</p></aside>
      </div>

      <section className="training-review"><div><span>CIERRE DEL BLOQUE</span><h2>Feedback y decisiones para la siguiente rotación</h2><p>Reemplaza las preguntas laterales del Excel con información ordenada y recuperable.</p></div><div className="training-review-grid"><label>Fatiga general (1–10)<input type="number" min="1" max="10" value={draft.review?.general_fatigue || ""} onChange={event => mutate(program => { program.review ??= { session_fatigue: [] }; program.review.general_fatigue = Number(event.target.value) || undefined; })}/></label><label>Tolerancia al volumen (1–10)<input type="number" min="1" max="10" value={draft.review?.volume_tolerance || ""} onChange={event => mutate(program => { program.review ??= { session_fatigue: [] }; program.review.volume_tolerance = Number(event.target.value) || undefined; })}/></label><label>Dolor muscular o molestias<textarea rows={2} value={draft.review?.soreness ?? ""} onChange={event => mutate(program => { program.review ??= { session_fatigue: [] }; program.review.soreness = event.target.value; })}/></label><label>Sesión más demandante<textarea rows={2} value={draft.review?.hardest_session ?? ""} onChange={event => mutate(program => { program.review ??= { session_fatigue: [] }; program.review.hardest_session = event.target.value; })}/></label><label>Ejercicios incómodos o con mala técnica<textarea rows={2} value={draft.review?.uncomfortable_exercises ?? ""} onChange={event => mutate(program => { program.review ??= { session_fatigue: [] }; program.review.uncomfortable_exercises = event.target.value; })}/></label><label>Ejercicios que conviene mantener<textarea rows={2} value={draft.review?.keep_exercises ?? ""} onChange={event => mutate(program => { program.review ??= { session_fatigue: [] }; program.review.keep_exercises = event.target.value; })}/></label></div></section>
