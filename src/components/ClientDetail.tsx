...
export function ClientDetail() {
  ...
  const animateMacroChange = (update: () => void) => {
    if (document.startViewTransition && !window.matchMedia("(prefers-reduced-motion: reduce)").matches) {
      document.documentElement.classList.add("macro-morphing");
      const transition = document.startViewTransition(() => flushSync(update));
      void transition.finished.catch(() => {}).finally(() => document.documentElement.classList.remove("macro-morphing"));
    } else update();
  };
  ...
  const macroEditor = editResult && (
          ...
          <div id="macro-editor" style={{ viewTransitionName: "macro-cards" }} className="px-5 pb-5 pt-4 border-t border-gray-100 dark:border-gray-800">
  ...
  return (
    <div className="client-detail-page"><Link className="care-entry" href={`/clients/${client.id}/care`}>Portal, seguimiento y acceso del cliente →</Link>
      ...
      {macroView && !result && (
        <div key={macroView.id} className="mb-4 macro-saved-summary" style={{ viewTransitionName: "macro-cards" }}>
