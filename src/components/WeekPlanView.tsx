...
export function WeekPlanView() {
  ...
  const handleCreateDay = (index: number) => {
    const dayMacros = calculateDayMacros(baseMacros, days[index]);
  ...
  const handleSaveWeek = () => {
    ...
    db.saveWeekPlan({
      ...
      day_plans: days.map((carb_day, i) => ({ day: i, carb_day, meal_plan_id: planIds[i] })),
    });
  };
    ...
    <div>
      ...
      <div className="grid grid-cols-7 gap-3 mb-6">
        {days.map((day, i) => {
          ...
          return (
            <div key={i} className="bg-gradient-to-b from-gray-50 to-white dark:from-gray-800 dark:to-gray-800/50 rounded-xl border border-gray-200 dark:border-gray-700 shadow-sm p-3.5 text-center">
              ...
              <p className="text-[10px] text-gray-400 dark:text-gray-500 mt-1.5">{adj.label}</p>
