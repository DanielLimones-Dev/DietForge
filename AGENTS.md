...
- `Calcular Macros` crea únicamente un borrador editable; solo `Guardar macros` persiste una evaluación y nunca crea un check-in. Cancelar descarta el borrador. El resultado temporal se muestra una sola vez dentro de la calculadora y, al guardarlo, pasa con transición al historial superior. Las evaluaciones se conservan como historial navegable, con la más reciente en la posición 1. Peso y Grasa corporal comparan los dos registros más recientes por fecha, ya sean evaluación o check-in; al compartir fecha prevalece el check-in. El panel biométrico reúne ambas métricas y gráficas; promedios, adherencia, tendencia y demás seguimiento propio del check-in permanecen separados.
...
- El selector de fases siempre incluye `peak_week`; es la entrada visible al calendario y debe mostrar sus factores propios antes de abrir el simulador.
...
- La biblioteca incluida es estática y trazable internamente a la hoja `Base de datos` del Excel de referencia: 183 ejercicios únicos, sin videos predeterminados. En la interfaz se integra bajo “Mis ejercicios” y no se muestra el nombre histórico del archivo. Cada coach decide si agrega YouTube o un archivo privado; ese contenido usa `video_custom`, `video_url` y `TrainingProgram.resources`. Ver `docs/TRAINING_MEDIA.md`. Personalizar una rutina no modifica la biblioteca global.
- Renovar acceso suma 1, 3 o 12 meses desde el vencimiento vigente si sigue activo, o desde la fecha actual si venció.
- Si una cuenta ya existe en Supabase Auth, una renovación conserva su contraseña y no vuelve a enviar invitación. Si tuvo un periodo pagado y vence, queda bloqueada hasta una nueva renovación; la prueba histórica no puede reactivarla.
- Eliminar un coach exige confirmación explícita de su correo en la interfaz y autorización administrativa en servidor. La operación elimina workspace, actividad, acceso, suscripciones, videos privados e identidad Auth; nunca permitir eliminar la cuenta administradora.
...
- Los controles numéricos editables de macros mantienen un texto temporal separado del número validado: deben permitir borrar el último dígito, bloquear el guardado si quedan vacíos y restaurar el último valor válido al perder foco. Los botones −/+ usan pasos de 50 kcal para calorías y 1 g para macros.
- En rutinas, los ejercicios conservan formato de filas legibles con encabezados; el resumen de series permanece por semana y el feedback vive junto al editor semanal para revisar prescripción y sensaciones en la misma vista.
