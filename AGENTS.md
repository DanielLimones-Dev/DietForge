...
- La biblioteca incluida es estática y trazable internamente a la hoja `Base de datos` del Excel de referencia: 430 ejercicios únicos, sin videos predeterminados. En la interfaz se integra bajo “Mis ejercicios” y no se muestra el nombre histórico del archivo. Cada coach decide si agrega YouTube o un archivo privado; ese contenido usa `video_custom`, `video_url` y `TrainingProgram.resources`. Ver `docs/TRAINING_MEDIA.md`. Personalizar una rutina no modifica la biblioteca global.
